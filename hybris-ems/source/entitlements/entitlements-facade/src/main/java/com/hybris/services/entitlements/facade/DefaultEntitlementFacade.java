...
public class DefaultEntitlementFacade implements EntitlementFacade
{
	...
	{
		final List<Condition> result = new ArrayList<>(conditionDataList.size());
		for (final ConditionData item : conditionDataList)
		{
	...
	{
		final List<Criterion> result = new ArrayList<>(criterionDataList.size());
		for (final CriterionData item : criterionDataList)
		{
			result.add(converter.convert(item));
		}
		return result;
