...
public class OCCServiceHelper implements ContentServiceHelper {
    ...
    @Override
    public boolean authenticateAsTrustedClient(final ResponseReceiverEmpty responseReceiver, final String requestId, final QueryLogin queryLogin, final boolean shouldUseCache, final List<View> viewsToDisable, final OnRequestListener onRequestListener) {

        // Extended authenticate = authenticate + call to get extended role
        ResponseReceiverEmpty responseReceiverLoginExtended = new ResponseReceiverEmpty() {

            @Override
            public void onResponse(final Response<EmptyResponse> responseUser) {

                // We set the extended role for the user
                setTrustedClientRoleForUser(responseReceiver, requestId, shouldUseCache, viewsToDisable, onRequestListener);
            }
        ...
        return authenticate(responseReceiverLoginExtended, requestId, queryLogin, shouldUseCache, viewsToDisable, onRequestListener);
    }

    @Override
    public boolean setTrustedClientRoleForUser(final ResponseReceiverEmpty responseReceiver, final String requestId, boolean shouldUseCache, List<View> viewsToDisable, OnRequestListener onRequestListener) {
        if (mUserInformation.isAnonymous()) {
            throw new IllegalStateException("No user is currently logged in, please log in or use the associated method for guest user setTrustedClientRoleForGuest(...).");
        }

        Map<String, Object> parameters = buildLoginParameters(null, Constants.Login.CLIENT_ID_TRUSTED_CLIENT, Constants.Login.GRANT_TYPE_LOGIN_TRUSTED_CLIENT);

        // We want to save the extended token before returning the response
        ResponseReceiver<UserInformation> responseReceiverBeforeCallback = new ResponseReceiver<UserInformation>() {
            @Override
            public void onResponse(Response<UserInformation> response) {
                String savedToken = mUserInformation.getSecureRefreshToken();
                String savedCartId = mUserInformation.getCartId();

                saveUserInformation(response.getData(), mUserInformation.getUserId());
                mUserInformation.setRefreshToken(savedToken);
        ...
        return execute(responseReceiverBeforeCallback, DataConverter.Helper.build(UserInformation.class, ErrorList.class, null),
                shouldUseCache, requestId, UrlHelper.getWebserviceTokenUrl(mContext, mConfiguration), parameters, null, true,
                HttpUtils.HTTP_METHOD_POST, viewsToDisable, onRequestListener);

    }

    @Override
    public boolean setTrustedClientRoleForGuest(final ResponseReceiverEmpty responseReceiver, final String requestId, boolean shouldUseCache, List<View> viewsToDisable, OnRequestListener onRequestListener) {
